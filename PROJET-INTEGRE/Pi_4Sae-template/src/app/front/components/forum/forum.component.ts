        ...
        <section class="posts-list">
          ...
          <div class="post-card" *ngFor="let p of filteredPosts" [class.bookmarked-card]="p.bookmarked">
            ...
            <div class="post-audio" *ngIf="p.audioUrl">
              ...
              <audio controls [src]="'/api/forum-files' + p.audioUrl"></audio>
            </div>
...
export class ForumComponent implements OnInit {
  ...
  openHistory(p: ForumPost) {
    ...
    this.forumService.getPostHistory(p.id!).subscribe({
      next: (data: PostEditHistory[]) => { this.history = data; this.historyLoading = false; },
  ...
  onSearch() {
    ...
    if (q) {
      this.forumService.search(this.searchQuery.trim()).subscribe((results: ForumPost[]) => {
        this.posts = results;
        this.applyFilter();
      });
    } else {
  ...
  toggleBookmark(p: ForumPost) {
    this.forumService.toggleBookmark(p.id!).subscribe((updated: ForumPost) => {
  ...
  likeReply(reply: Reply, postId: number) {
    this.forumService.likeReply(reply.id!).subscribe((updated: Reply) => {
  ...
  toggleReplies(p: ForumPost) {
    ...
    if (this.openReplies[id] && !this.repliesMap[id]) {
      this.forumService.getReplies(id).subscribe((r: Reply[]) => this.repliesMap[id] = r);
    }
  }
  ...
  submitReply(postId: number) {
    ...
    this.forumService.addReply(postId, rf).subscribe((reply: Reply) => {
  ...
  react(p: ForumPost, emoji: string) {
    this.forumService.addReaction(p.id!, emoji).subscribe((updated: ForumPost) => {
      const idx = this.posts.findIndex(x => x.id === p.id);
