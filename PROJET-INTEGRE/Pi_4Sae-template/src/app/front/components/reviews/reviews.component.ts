...
export class ReviewsComponent implements OnInit {
  ...
  load() {
    ...
    this.reviewService.getAll().subscribe({
      next: (data: Review[]) => { this.reviews = data; this.loading = false; this.applyFilter(); },
      error: () => { this.loading = false; }
    });
    this.reviewService.getAverageRating().subscribe({
      next: (data: { averageRating: number }) => { this.averageRating = data.averageRating; }
    });
  }
  ...
  onSearch() {
    if (this.searchQuery.trim()) {
      this.reviewService.search(this.searchQuery.trim()).subscribe((results: Review[]) => {
  ...
  openHistory(r: Review) {
    ...
    this.reviewService.getReviewHistory(r.id!).subscribe({
      next: (data: ReviewEditHistory[]) => { this.history = data; this.historyLoading = false; },
  ...
  toggleBookmark(r: Review) {
    this.reviewService.toggleBookmark(r.id!).subscribe((updated: Review) => {
  ...
  addHelpfulVote(r: Review) {
    this.reviewService.addHelpfulVote(r.id!).subscribe((updated: Review) => {
      const idx = this.reviews.findIndex(x => x.id === r.id);
  ...
  async translate(r: Review) {
    ...
    try {
      const res = await fetch(`/api/reviews/${r.id}/translate?targetLang=${target}`, { method: 'POST' });
