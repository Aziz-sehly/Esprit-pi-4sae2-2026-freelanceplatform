...
//  1. AdminGuard applied to the /back prefix route (protects all children)
//  2. Added back/admin-services and back/admin-contracts lazy routes
//  3. Added front/services/:slug route for service detail page
//
// Only the modified/added sections are shown below.
// Merge these into your existing routes array.

// ── NEW IMPORTS to add at the top ────────────────────────────────────────────
// import { AdminGuard } from './guards/admin.guard';

// ── REPLACE your existing back routes block with this ─────────────────────
...
/*
  {
    ...
    children: [
      ...
      // ── Freelance Platform Admin ──────────────────────────────────────────
      {
        path: 'admin-projects',
        loadComponent: () => import('./admin/admin-projects/admin-projects.component')
          .then(m => m.AdminProjectsComponent)
      },
      {
        path: 'admin-proposals',
        loadComponent: () => import('./admin/admin-proposals/admin-proposals.component')
          .then(m => m.AdminProposalsComponent)
      },
      {
        path: 'admin-services',         // ← NEW
        loadComponent: () => import('./admin/admin-services/admin-services.component')
          .then(m => m.AdminServicesComponent)
      },
      {
        path: 'admin-contracts',        // ← NEW
        loadComponent: () => import('./admin/admin-contracts/admin-contracts.component')
          .then(m => m.AdminContractsComponent)
      },

      // ... all other existing back-office routes go here, unchanged ...
    ]
  },
*/

// ── ADD this inside your existing front routes (app.routes for front) ──────
// If your front routes are in a separate file (front/app.routes.ts), add:
/*
  {
    path: 'services',
    loadComponent: () => import('./pages/services/service-list/service-list.component')
      .then(m => m.ServiceListComponent)
  },
  {
    path: 'services/:slug',              // ← NEW — service detail by slug
    loadComponent: () => import('./pages/services/service-detail/service-detail.component')
      .then(m => m.ServiceDetailComponent)
  },
*/

// ─────────────────────────────────────────────────────────────────────────────
// Full routes array (copy-paste ready, replaces your current app.routes.ts)
// ─────────────────────────────────────────────────────────────────────────────
...
// Pages (abbreviated — keep all your existing imports)
...
export const routes: Routes = [
  ...
  {
    path: 'back',
    canActivate: [AdminGuard],           // ← ROLE GUARD: only ADMIN users
    ...
    children: [
      ...
      {
        path: 'admin-services',               // ← NEW
        loadComponent: () => import('./admin/admin-services/services.component')
          .then(m => m.AdminServicesComponent)
      },
      {
        path: 'admin-contracts',              // ← NEW
      ...
      { path: 'ecommerce-page', component: EcommercePageComponent, children: [
      ...
      { path: 'crm-page', component: CrmPageComponent, children: [
      ...
      { path: 'project-management-page', component: ProjectManagementPageComponent, children: [
      ...
      { path: 'lms-page', component: LmsPageComponent, children: [
      ...
      { path: 'help-desk-page', component: HelpDeskPageComponent, children: [
      ...
      { path: 'invoices', component: InvoicesPageComponent, children: [
        { path: '', component: InvoicesComponent },
        { path: 'invoice-details', component: InvoiceDetailsComponent }
      ]},

      { path: 'social', component: SocialPageComponent, children: [
      ...
      { path: 'charts', component: ApexchartsComponent, children: [
      ...
      { path: 'ui-kit', component: UiElementsComponent, children: [
  ...
  // ── FRONT-END (public + authenticated users) ─────────────────────────────
  {
    path: 'front',
