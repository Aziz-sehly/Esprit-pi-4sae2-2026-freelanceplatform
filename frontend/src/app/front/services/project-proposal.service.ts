...
export class ProjectProposalService {
  ...
  private readonly baseUrl = 'http://localhost:8085/microservice-proposal/proposal';
  private readonly projectBaseUrl = 'http://localhost:8085/microservice-project/project';
  ...
  generateCoverLetter(req: CoverLetterRequest): Observable<CoverLetterResponse> {
    return this.http.post<CoverLetterResponse>(
      `${this.baseUrl}/ai-cover-letter`, req
    ).pipe(
      catchError((err) => {
  ...
  private dtoToProposal(dto: any): ProjectProposal {
    ...
    return {
      ...
      // Payment Structure
      paymentStructure: (raw.paymentStructure ?? raw.payment_structure ?? 'FIXED') as PaymentStructureType,
      hourlyRate: raw.hourlyRate ?? raw.hourly_rate,
      estimatedHoursPerWeek: raw.estimatedHoursPerWeek ?? raw.estimated_hours_per_week,
      milestoneCount: raw.milestoneCount ?? raw.milestone_count,
      milestoneDetails: raw.milestoneDetails ?? raw.milestone_details,
      
      counterOfferPrice: raw.counterOfferPrice ?? raw.counter_offer_price,
      counterOfferMessage: raw.counterOfferMessage ?? raw.counter_offer_message,
  ...
  create(input: Omit<ProjectProposal, 'id' | 'status' | 'submittedAt'> & { 
  ...
  }): Observable<ProjectProposal> {
    const payload = {
      projectId: input.projectId,
      freelancerId: input.freelancerId,
      proposedPrice: input.proposedBudget,
      deliveryDays: input.deliveryDays ?? (input.estimatedDuration ? parseInt(input.estimatedDuration, 10) : 0),
      coverLetter: input.coverLetter,
      status: 'PENDING',
      isInvited: input.isInvited ?? false,
      revisionsOffered: input.revisionsOffered ?? 0,
      paymentStructure: input.paymentStructure ?? 'FIXED',
  ...
  accept(proposalId: number): Observable<ProjectProposal | undefined> {
    return this.http.post<any>(`${this.baseUrl}/Accept/${proposalId}`, {}).pipe(
      switchMap(() => this.getById(proposalId)),
      switchMap((proposal) => {
        if (!proposal) return of(undefined);
        return this.projectService.update(proposal.projectId, { status: Status.IN_PROGRESS }).pipe(
          map(() => proposal),
  ...
  reject(proposalId: number): Observable<ProjectProposal | undefined> {
    return this.http.post<any>(`${this.baseUrl}/Reject/${proposalId}`, {}).pipe(
      map((dto) => this.dtoToProposal(dto)),
      catchError((err) => {
