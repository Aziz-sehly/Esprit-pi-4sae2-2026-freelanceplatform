...
export class DisputeCenterComponent implements OnChanges {
  ...
  loadData(): void {
    ...
    this.loading = true;
    this.disputeService.list(this.contract.id, this.contract).subscribe({
      next: (disputes) => {
  ...
  create(): void {
    ...
    this.loading = true;
    this.disputeService.create(payload, this.contract).subscribe({
      next: () => {
        this.loading = false;
        this.form.reset({
          title: '',
  ...
  submitAction(dispute: DisputeResponse): void {
    ...
    const request$ =
      this.actionMode === 'review'
        ? this.disputeService.review(dispute.id, { note }, this.contract)
        : this.actionMode === 'resolve'
          ? this.disputeService.resolve(dispute.id, { note }, this.contract)
          : this.disputeService.reject(dispute.id, { note }, this.contract);
