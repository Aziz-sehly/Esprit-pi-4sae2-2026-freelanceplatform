...
export class ActivityCenterComponent implements OnChanges {
  ...
  load(): void {
    ...
    forkJoin({
      ...
      disputes: this.disputeService.list(this.contract.id, this.contract),
      contractResources: this.resourceService.list('CONTRACT', this.contract.id),
      chatMessages: this.chatService.listContractMessages(this.contract),
    }).subscribe({
    ...
    const items: ActivityItem[] = [
      {
        ...
        description: `${this.contract.clientName ?? 'Client #' + this.contract.clientId} created the contract for ${this.contract.freelancerName ?? 'Freelancer #' + this.contract.freelancerId}.`,
        category: 'Contract',
        tone: 'neutral',
      },
      {
