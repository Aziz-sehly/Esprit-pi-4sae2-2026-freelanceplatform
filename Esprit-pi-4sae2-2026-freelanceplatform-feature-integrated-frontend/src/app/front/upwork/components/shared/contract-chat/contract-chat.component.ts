...
export class ContractChatComponent implements OnInit, OnChanges, AfterViewChecked, OnDestroy {
  ...
  /** Contrepartie (client ou freelancer) pour l’API message-service. */
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if ((changes['contractId'] || changes['receiverId'] || changes['senderId']) && this.contractId && this.receiverId) {
      this.lastMessageId = null;
      this.unreadCount = 0;
      this.ignoreNextMessageBatch = true;
      this.chatService.connect(this.contractId, this.senderId, this.receiverId, this.senderRole, this.senderName, this.receiverName);
      this.chatService.loadHistory(this.contractId, this.senderId, this.receiverId);
    }
  }
  ...
  toggle(): void {
    this.open = !this.open;
    if (this.open && this.contractId && this.receiverId) {
      this.unreadCount = 0;
      this.ignoreNextMessageBatch = true;
      this.chatService.connect(this.contractId, this.senderId, this.receiverId, this.senderRole, this.senderName, this.receiverName);
      this.chatService.loadHistory(this.contractId, this.senderId, this.receiverId);
  ...
  sendText(): void {
    ...
    this.loading = true;
    this.chatService.send(this.contractId, this.receiverId, {
      senderId: this.senderId,
      senderRole: this.senderRole,
      senderName: this.senderName,
      content,
    }, this.receiverName).subscribe({
  ...
  sendLink(): void {
    ...
    this.chatService.sendLink(this.contractId, this.receiverId, {
      senderId: this.senderId,
      senderRole: this.senderRole,
      senderName: this.senderName,
  ...
  attachmentUrl(message: ChatMessageResponse): string | null {
    if (message.attachmentType === 'FILE' && message.attachmentDownloadUrl) {
      return `${environment.messageApiBase}${message.attachmentDownloadUrl}`;
    }
