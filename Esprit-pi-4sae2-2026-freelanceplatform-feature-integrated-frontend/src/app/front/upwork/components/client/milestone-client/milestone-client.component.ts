...
export class MilestoneClientComponent {
    ...
    this.contractService.getById(this.contractId).subscribe({
      ...
      error: (err) => {
        ...
        const msg =
          ...
          err?.message ||
          (err?.status === 0
            ? 'Réseau ou CORS : vérifiez le gateway (8085), Eureka, et gatewayBaseUrl / milestoneApiBase dans environment.ts.'
            : `Erreur HTTP ${err?.status ?? '?'}. Contrat introuvable ou non autorisé.`);
  ...
  requestRevision(milestone: MilestoneResponse): void {
    ...
    this.loading = true;
    this.milestoneService.requestRevision(milestone.id, { feedback: this.revisionFeedback.trim(), actorId: 101 }).subscribe({
      next: () => {
        this.loading = false;
        this.revisionComposerFor = null;
        this.revisionFeedback = '';
