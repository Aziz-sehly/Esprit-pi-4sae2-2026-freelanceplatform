...
private readonly baseUrl = 'http://localhost:8085/microservice-project/project';
  ...
  aiSuggest(description: string, duration: string): Observable<AISuggestResponse> {
    ...
    return this.http.post<AISuggestResponse>(`${this.baseUrl}/ai-suggest`, body).pipe(
      ...
      catchError((err) => {
        console.error('AI suggest failed:', err.status, err.error);
        return of({ title: '', skills: '', category: '', budgetMin: 0, budgetMax: 0 });
      })
    );
  }
  ...
  create(project: Omit<Project, 'id'>): Observable<Project> {
    ...
    return this.http.post<any>(`${this.baseUrl}/Addproject`, payload).pipe(
      ...
      catchError((error) => {
        console.error('Create project error:', error.status, error.error);
  ...
  update(id: number, patch: Partial<Project>): Observable<Project | undefined> {
    ...
    return this.http.put<any>(`${this.baseUrl}/UpdateProject/${id}`, payload).pipe(
      ...
      catchError((error) => {
        console.error('Update project error:', error.status, error.error);
        return of(undefined);
      })
    );
  }
  ...
  delete(id: number): Observable<boolean> {
    return this.http.delete<void>(`${this.baseUrl}/DeleteProject/${id}`).pipe(
      ...
      catchError((error) => {
        console.error('Delete project error:', error.status, error.error);
        return of(false);
      })
    );
  }
  ...
  private dtoToProject(dto: any): Project {
    ...
    return {
      id:              Number(raw.id),
      // ✅ clientId is now a direct field — no more raw.client?.id fallback needed
      clientId:        Number(raw.clientId ?? raw.client_id ?? 0),
  ...
  private projectToPayload(patch: Partial<Project>): any {
    ...
    // ✅ Send clientId as a plain number — no longer wrapped in { client: { id } }
