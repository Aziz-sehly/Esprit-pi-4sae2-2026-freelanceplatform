...
export class ContractSigningComponent implements OnInit, AfterViewInit {
  ...
  ngAfterViewInit(): void {
    // Canvas lives inside *ngIf="!isLoading" — it is not in the DOM yet when this runs.
    this.scheduleInitCanvas();
  }

  /** Run after the signing UI is actually rendered (after loading finishes or tab switch). */
  private scheduleInitCanvas(): void {
    setTimeout(() => {
      this.initCanvasIfReady();
      requestAnimationFrame(() => this.initCanvasIfReady());
    }, 0);
  }

  private initCanvasIfReady(): void {
    if (this.activeTab !== 'draw' || !this.canvasRef?.nativeElement) {
      return;
    }
    this.initCanvas();
  }
