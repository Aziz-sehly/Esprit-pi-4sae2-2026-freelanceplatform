...
public class PaymentService {
    ...
    public PaymentResponse create(PaymentRequest req) {
        Payment saved = repo.save(buildPayment(req, PaymentStatus.PENDING));
        return toResponse(saved);
    }

    public CheckoutSessionResponse createCheckoutSession(PaymentRequest req) {
        Payment payment = repo.save(buildPayment(req, PaymentStatus.INITIATED));
    ...
    public PaymentResponse release(Long id) {
        Payment payment = repo.findById(id)
                .orElseThrow(() -> new NotFoundException("Payment not found: " + id));
    ...
    public PaymentResponse requestRefund(Long id, RefundRequest req) {
        Payment payment = repo.findById(id)
                .orElseThrow(() -> new NotFoundException("Payment not found: " + id));
        if (payment.getStatus() != PaymentStatus.FUNDED && payment.getStatus() != PaymentStatus.RELEASED) {
    ...
    private Payment buildPayment(PaymentRequest req, PaymentStatus status) {
