...
- `media-analysis-service` (port `8086`) — analyse des pièces jointes (images, PDF, texte, heuristique vidéo) pour les **insights** admin
...
3. Start `media-analysis-service` (requis pour les scores de preuves basés sur le contenu dans `GET /api/disputes/{id}/insights` ; sinon le litige-service ignore l’analyse)
4. Start `dispute-service`
5. Start `api-gateway`
...
- Inter-service: OpenFeign (`dispute-service` -> `message-service`, `dispute-service` -> `media-analysis-service`, `media-analysis-service` -> `message-service` pour télécharger les fichiers)
